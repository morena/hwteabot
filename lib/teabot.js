...
TeaBot.prototype._runTimer =function(teabot, channel) {
   // do stuff
   var self = teabot;

   console.log("teabot.teaMaker = " + teabot.teaMaker);
	...
	if(interval === 0){
		...
		reply = '@channel, @'+teaMaker.name+ ' wants to make hwtea! You now have 3 minutes to send me your tea order in a DM';
		self.postMessageToChannel(channel.name, reply, {as_user:true});
		interval++;
		self.interval = interval;
		
		console.log("self.interval updated to " + self.interval);

		console.log("I want to run it again now");
	   // and schedule a repeat
	   setTimeout(self._runTimer.bind(TeaBot, channel), 180000); //180000); //3 minutes
	}else{
		self.postMessageToUser( teaMaker.name, message );
		//destroy order
		self.teaOrder = [];
		console.log("The tea round has ended!");
		reply = '@channel, the tea round is finished! @'+teaMaker.name+ ' has got you order, your tea is coming soon!';
		self.postMessageToChannel(channel.name, reply, {as_user:true});
		self.interval = 0;
	}
}
